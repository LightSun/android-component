...
public class TestLogic {
  ...
  public void testNormalAsync() {
    LogicTask task =
        LogicTask.of(TAG_OK, mAction, new LogicParam().setData("testNormalAsync"))
            .schedulerOn(Schedulers.ASYNC)
            .delay(3000);
    System.out.println(
        "============ testNormalAsync() >>> start time = " + Schedulers.getCurrentTime());
    mLm.performSequence(task, 0, null);
  }

  public void testNormal() {
    LogicTask task =
        LogicTask.of(TAG_OK, mAction, new LogicParam().setData("testNormal"))
  ...
  public void testCancel() {
    LogicTask task1 =
        LogicTask.of(TAG_OK, mAction, new LogicParam().setData("testCancel_1"))
            .schedulerOn(Schedulers.ASYNC)
            .delay(2500);
    LogicTask task2 =
        LogicTask.of(TAG_ERROR, mAction, new LogicParam().setData("testCancel_2"))
            .schedulerOn(Schedulers.newAsyncScheduler())
            .delay(3000);
    System.out.println("============ testCancel() >>> start time = " + Schedulers.getCurrentTime());
    final int key =
        mLm.performParallel(new LogicTask[] {task1, task2}, 0, new LogListenerImpl("testCancel"));

    Logger.i(TAG, "testCancel", "key = " + key);
    Schedulers.newAsyncScheduler()
        .postDelay(
            2400,
            new Runnable() {
              @Override
              public void run() {
                mLm.cancel(key);
                assert (mAction.getLogicParameter(0)) == null;
              }
            });
  }
  ...
  public void testCancelSequence() {
    LogicTask task1 =
        LogicTask.of(TAG_OK, mAction, new LogicParam().setData("testCancelSequence_1"))
            .schedulerOn(Schedulers.ASYNC)
            .delay(2500);
    LogicTask task2 =
        LogicTask.of(TAG_ERROR, mAction, new LogicParam().setData("testCancelSequence_2"))
            .schedulerOn(Schedulers.newAsyncScheduler())
            .delay(3000);
    System.out.println(
        "============ testCancelSequence() >>> start time = " + Schedulers.getCurrentTime());
    final int key =
        mLm.performSequence(
            new LogicTask[] {task1, task2}, 0, new LogListenerImpl("testCancelSequence"));
    Logger.i(TAG, "testCancel", "key = " + key);
    Schedulers.newAsyncScheduler()
        .postDelay(
            2600,
  ...
  public void testScheduler() {
    LogicTask task1 =
        LogicTask.of(TAG_OK, mAction, new LogicParam().setData("testScheduler_1"))
            .schedulerOn(Schedulers.ASYNC)
            .observeOn(Schedulers.newAsyncScheduler())
            .delay(2500);
    LogicTask task2 =
        LogicTask.of(TAG_ERROR, mAction, new LogicParam().setData("testScheduler_2"))
            .schedulerOn(Schedulers.newAsyncScheduler())
            .delay(3000);
